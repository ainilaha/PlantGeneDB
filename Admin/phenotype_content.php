...
$where = "WHERE Species LIKE ? OR Treatment LIKE ? OR Source LIKE ?";
$params = [$search, $search, $search];
$param_types = "sss";

// 获取总记录数
$count_sql = "SELECT COUNT(*) FROM Phenotype $where";
...
// 获取数据
$sql = "SELECT * FROM Phenotype $where 
        ORDER BY id DESC 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    try {
        ...
        $delete_stmt = $conn->prepare("DELETE FROM Phenotype WHERE id = ?");
        $delete_stmt->bind_param("i", $delete_id);
        if ($delete_stmt->execute()) {
            $success = "Record deleted successfully";
            // 刷新当前页数据
            header("Location: phenotype_content.php?page=$page");
            exit();
        }
    } catch (Exception $e) {
        ...
        <div class="data-table p-3">
            ...
            <form class="search-box">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="搜索物种、处理或来源..."
                    ...
                    <?php while($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td class="action-btns">
                                <a href="phenotype_edit.php?id=<?= $row['id'] ?>"
